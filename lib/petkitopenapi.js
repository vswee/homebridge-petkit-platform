...
class PetkitOpenAPI {
  ...
  async login(username, password) {
    ...
    console.log("pass:",pass, "string crypto:",crypto.MD5(pass).toString(), "crypto:",crypto.MD5(pass))
  ...
  async get_assets() {
    let groupID = await this.post('/group/family/list').result.groupId;
    let res = await this.post('/discovery/device_roster', {day: Date.now(), groupId: groupID});
