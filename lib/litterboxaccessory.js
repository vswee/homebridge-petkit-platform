...
class LitterBoxAccessory extends BaseAccessory {
  ...
  getDay() {
    let now = new Date();
    let leadingZeroMonth = now.getMonth() > 10 ? '' : '0'
    let leadingZeroDay = now.getDate() > 10 ? '' : '0'
    let day = String(now.getFullYear()) + leadingZeroMonth + String(now.getMonth()) + leadingZeroDay + String(now.getDate());
    console.log(">>>>>>>>>>>>>>>>>>DAY:", day)
    return day
  }
  //addService function
